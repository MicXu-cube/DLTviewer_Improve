...
#ifdef XWBLOG
    qDebug() << "xwb:DltFileIndexer::index before. num =" << num;
#endif
...
#ifdef XWBLOG
    qDebug() << "xwb[import]:DltFileIndexer::index " << indexAllList.size();
    //下述打印在此处无效，因为它是先执行此函数，在进行set index的，所以要等待这个函数退出之后相关数值才能解除
//    qDebug() << "xwb[import]:DltFileIndexer::index " << dltFile->getFileMsgNumber(num);
#endif
        return true;
    }
...
#ifdef XWBLOG
    qDebug() << "xwb:DltFileIndexer::index f.size =" << f.size() << "mode = " << f.size()/2000;
#endif
...
#ifdef XWBLOG
    qDebug() << "xwb:DltFileIndexer do-roll readresult =" << readresult << "pos = " << pos;
#endif
        if (length >= 0)
        {
           length = readresult;
        ...
        //下述的for-roll是在处理这读出的readresult所有数据
        for(number=0;number < length;number++)
        {
            abspos= pos+number;
//#ifdef XWBLOG
//    qDebug() << "xwb:DltFileIndexer for-roll pos =" << pos << "number =" << number << "abspos =" << abspos
//             << "lastFound="<< lastFound << "data= " << data[number]
//             << "current_message_pos =" << current_message_pos << " next_message_pos = " << next_message_pos << "counter_header =" << counter_header
//             ;
//#endif
...
#ifdef XWBLOG
    qDebug() << "xwb:DltFileIndexer do-roll length =" << length << "pos = " << pos;
#endif
    }
...
#ifdef XWBLOG
    qDebug() << "xwb:DltFileIndexer::index after";
#endif
    return true;
}
...
{
#ifdef XWBLOG
    qDebug() << "xwb:DltFileIndexer::indexFilter before" << __FILE__ << __LINE__;
#endif
...
#ifdef XWBLOG
    qDebug() << "xwb:DltFileIndexer::indexFilter filterCacheEnabled = " << filterCacheEnabled << "mode = "<< mode << __FILE__ << __LINE__;
#endif
...
#ifdef XWBLOG
    qDebug() << "xwb[import]:DltFileIndexer::indexFilter for-roll before dltFilesize = " << dltFile->size() << "modvalue = " << modvalue << __FILE__ << __LINE__;
    qint64 xwbtest_readmsgconut = 0;
#endif
...
#ifdef XWBLOG
    qDebug() << "xwb[import]:DltFileIndexer::indexFilter for-roll inter useIndexerThread = " << useIndexerThread << __FILE__ << __LINE__;
#endif
...
#ifdef XWBLOG
    qDebug() << "xwb[import]:DltFileIndexer::indexFilter for-roll after dltFilesize = " << dltFile->size() << "xwbtest_readmsgconut = " << xwbtest_readmsgconut << __FILE__ << __LINE__;
#endif
    emit(progress(100));
    // destroy threads
    if(true == useIndexerThread)
...
void DltFileIndexer::run()
{
...
#ifdef XWBLOG
    qDebug() << "xwb:DltFileIndexer::run before" << __FILE__ << __LINE__;
#endif
...
#ifdef XWBLOG
    qDebug() << "xwb[import]:DltFileIndexer::run |index| mode =" << mode << __FILE__ << __LINE__;
    qDebug() << "xwb[import]:DltFileIndexer::run |index| filesize =" << dltFile->getNumberOfFiles() << __FILE__ << __LINE__;
#endif
    ...
    // indexFilter
#ifdef XWBLOG
    qDebug() << "xwb[import]:DltFileIndexer::run |indexFilter|" << "mode =" << mode << __FILE__ << __LINE__;
#endif
    ...
    */
#ifdef XWBLOG
    qDebug() << "xwb:DltFileIndexer::run after" << __FILE__ << __LINE__;
#endif
}
...
bool DltFileIndexer::loadIndexCache(QString filename)
{
    ...
    {
#ifdef XWBLOG
    qDebug() << "xwb:DltFileIndexer::loadIndexCache loadIndex() fail, return false";
#endif
...
{
#ifdef XWBLOG
    qDebug() << "xwb:DltFileIndexer::saveIndexCache before";
#endif
    QString filenameCache;
...
{
#ifdef XWBLOG
    qDebug() << "xwb:DltFileIndexer::loadFilterIndexCache before" << __FILE__ << __LINE__;
#endif
    QString filenameCache;
...
bool DltFileIndexer::loadIndex(QString filename, QVector<qint64> &index)
{
    ...
    {
#ifdef XWBLOG
    qDebug() << "xwb:DltFileIndexer::loadIndex Loading index file fail, return false";
#endif
