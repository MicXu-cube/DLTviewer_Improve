...
#ifdef XWBLOG
    qDebug() << "xwb:QDltFile::open. append = " << append;
#endif
...
#ifdef XWBLOG
    qDebug() << "xwb:QDltFile::createIndex before";
#endif
...
#ifdef XWBLOG
    qDebug() << "xwb:QDltFile::createIndex after";
#endif
    return ret;
}
...
bool QDltFile::updateIndex()
{
...
#ifdef XWBLOG
    qDebug() << "xwb:before QDltFile::updateIndex,files.size =" << files.size();
#endif
    for(int numFile=0;numFile<files.size();numFile++)
    {
        ...
        static const int READ_BUF_SZ = 1024 * 1024;//xwbtest
...
#ifdef XWBLOG
    qDebug() << "xwb:after QDltFile::updateIndex,files.size =" << files.size();
#endif
    mutexQDlt.unlock();
...
bool QDltFile::getMsg(int index,QDltMsg &msg) const
{
    ...
    {
#ifdef XWBLOG
    qDebug() << "xwb:QDltFile::getMsg is empty, return false. index = " << index << __FILE__ << __LINE__;
#endif
        return false;
    }
